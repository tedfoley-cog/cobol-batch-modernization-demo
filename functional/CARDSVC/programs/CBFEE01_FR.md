...
- Inputs: FEE-WORK-AREA request fields; FEEPARM control cards (FD 80-byte KEYWORD/VALUE records, `CBFEE01.cbl:43-57`) — **no rate, threshold or cap is coded in the procedure division**; amounts come from `CARDSVC.FEE_SCHEDULE`, everything else from cards (`CBFEE01.cbl:15-18`). **Absent artifact:** no JCL in the estate allocates a FEEPARM DD — CBCRD05AJ (the only dispatching job) allocates only STEPLIB/CYCLCTL/FEEAUDIT (`CBCRD05AJ.jcl:36-55`); the card producer/allocation is missing from the repo (analysis §8, stream FR §5.3 item 14). A failed FEEPARM OPEN is fatal, not a soft default: `WS-FATAL-SW` is set with 'FEEPARM OPEN FAILED' (`CBFEE01.cbl:248-255`) and 9900-SET-RETURN returns BR-RETURN-CD 12 (`CBFEE01.cbl:731-733`), which CBCRD05A abends as U0505 — so the committed JCL alone could never have run the branch, confirming the artifact is absent rather than optional. The target fee parameter table/config becomes the authoritative source, with initial values confirmed with the business at wave-2 sign-off.
