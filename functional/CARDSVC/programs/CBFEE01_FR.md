...
- Inputs: FEE-WORK-AREA request fields; FEEPARM control cards (FD 80-byte KEYWORD/VALUE records, `CBFEE01.cbl:43-57`) — **no rate, threshold or cap is coded in the procedure division**; amounts come from `CARDSVC.FEE_SCHEDULE`, everything else from cards (`CBFEE01.cbl:15-18`). **Absent artifact:** no JCL in the estate allocates a FEEPARM DD — CBCRD05AJ (the only dispatching job) allocates only STEPLIB/CYCLCTL/FEEAUDIT (`CBCRD05AJ.jcl:36-55`); the card producer/allocation is missing from the repo (analysis §8, stream FR §5.3 item 14). The target fee parameter table/config becomes the authoritative source, with initial values confirmed with the business at wave-2 sign-off.
