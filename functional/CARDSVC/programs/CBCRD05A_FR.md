...
- In: CYCLCTL (`CBCRD05AJ.jcl:50`). Note: the dispatched fee handlers expect a FEEPARM card dataset, but CBCRD05AJ allocates no FEEPARM DD (absent artifact — see CBFEE01_FR §2, stream FR §5.3 item 14). Statically, running as committed would be **fatal on the first dispatch**: every handler's FEEPARM OPEN failure sets its fatal switch (`CBFEE01.cbl:248-255`, `CBFEE02.cbl:227-234`, `CBFEE03.cbl:253-260`) → handler BR-RETURN-CD 12 → CBCRD05A's `WHEN OTHER` U0505 abend (`CBCRD05A.cbl:531-536`) — so the estate must have supplied FEEPARM by a mechanism not in the repo; "legacy parity" for the fee branch cannot be defined from the committed JCL alone.
