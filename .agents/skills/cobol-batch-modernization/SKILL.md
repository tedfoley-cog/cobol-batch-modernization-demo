...
3. Emit a stream inventory from the JCL chain, PROC, programs, datasets, and Db2 access.
4. Extract EARS FRs, ISO 25010 NFRs, and the data layer against the customer schemas. Cite actual
   source spans; never fabricate missing DCLGEN members.
5. Run the deterministic feedback loop:
...
6. Create a migration plan with foundation, like-to-like, new-design, and depend-on-new chunks,
   dependencies, waves, and sign-off.
7. Migrate one program to Java 17/Spring Batch idioms.
8. Run parity from the committed fixtures:

   ```sh
   make parity
   ```

9. Rebuild the evidence viewer:

   ```sh
   make report
   ```

10. Finale: Devin opens the generated UI in its own browser and click-tests every tab, the
